...
# Based on https://gist.githubusercontent.com/samuelcolvin/ca8b429504c96ee738d62a798172b046/raw/439d0e71003c084adabaf0a0a593fbb715507e14/dns_server.py
...
SERIAL = int((datetime.utcnow() - EPOCH).total_seconds())  # The serial is used to determine already fetched entries. Needs to change every time!
...
#                    60 * 60 * 1,  # minimum
#                ),)
                # add non sensical short times to SOA
                args += ((
                    SERIAL,  # serial number
                    1,  # refresh
                    1,  # retry
...
resolver = Resolver()
servers = [
    DNSServer(resolver, port=7053, address='localhost', tcp=True),
    DNSServer(resolver, port=7053, address='localhost', tcp=False),
]
